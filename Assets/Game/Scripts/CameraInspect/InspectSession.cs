...
public class InspectSession
{
    ...
    public void Begin()
    {
        ...
        runner.StartCoroutine(Fly(go.transform, holdPoint.position, toRot, flyTime, () =>
        {
            ...
            if (targetBugAI != null)
            {
                ...
                Debug.Log($"[InspectSession] Showing bug hints. MultiHintController.Instance = {(MultiHintController.Instance != null ? "exists" : "NULL")}");
                ...
                {
                    Debug.LogWarning("[InspectSession] MultiHintController.Instance is NULL! Cannot show hints.");
                }
            }
            else
            {
                // Show only Put hint for non-bug inspectables: panel 0 (LMB)
                Debug.Log($"[InspectSession] Showing inspect hint. MultiHintController.Instance = {(MultiHintController.Instance != null ? "exists" : "NULL")}");
                if (MultiHintController.Instance != null)
                {
    ...
    public bool UpdateInput(bool exitPressed)
    {
        ...
        // Update hints based on inspected object type (UpdateInput)
        // NOTE: This is commented out because hints should only show once after animation, not every frame
        // if (targetBugAI != null)
        // {
        //     MultiHintController.Instance?.Show(0, 1);
        // }
