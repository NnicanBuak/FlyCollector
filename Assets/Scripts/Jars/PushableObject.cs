...
/// <summary>
/// Интерфейс для объектов, которые можно толкнуть
/// </summary>
...
/// <summary>
/// Компонент для объектов, которые можно толкнуть
/// Поддерживает объекты с JarSwing - толкает их сильнее вместо отцепления
/// </summary>
public class PushableObject : MonoBehaviour, IPushable
{
    ...
    [SerializeField] private Color pushHoverColor = new Color(255, 100, 0); // Оранжевый
    ...
    public void OnPushed(Vector3 force, Vector3 hitPoint)
    {
        ...
        {
            // Обычное откидывание для объектов без JarSwing
...
/// <summary>
/// Класс для управления толканием объекта
/// </summary>
public class PushSession
{
    ...
    public void Execute()
    {
        ...
        if (pushable != null)
        {
            // Используем IPushable интерфейс
            pushable.OnPushed(pushForce, hitPoint);
        }
        else
        {
            // Пытаемся толкнуть через JarSwing напрямую
